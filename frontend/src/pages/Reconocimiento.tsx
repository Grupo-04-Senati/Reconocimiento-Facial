...
import { reconocerRostro, healthCheck } from '../services/api'
      ...
      setResult({
        ...
        data: {
          ...
          detail: err.message?.includes('fetch')
            ? 'Backend no disponible. Ejecuta el servidor backend en localhost:8000'
            : err.response?.data?.detail || 'Error al procesar el reconocimiento',
        },
      })
    } finally {
      setLoading(false)
